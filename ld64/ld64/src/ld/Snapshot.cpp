...
#if __has_extension(blocks) || defined(USE_BLOCKS) // ld64-port
...
void Snapshot::copyFileToSnapshot(const char *sourcePath, const char *subdir, char *path) 
{
    ...
    int out_fd = open(path, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, mode);
...
void Snapshot::createSnapshot()
{
    if (fRootDir == NULL) {
        ...
        if (!fRecordKext) {
            ...
            int compileScript = open(buf, O_WRONLY|O_CREAT|O_TRUNC | O_BINARY, mode);
        ...
        int pidfile = open(path, O_WRONLY|O_CREAT|O_TRUNC | O_BINARY, mode);
        char pid_buf[32];
...
void Snapshot::writeCommandLine(bool rawArgs)
{
    ...
    if (!isLazy() && fRecordArgs) {
        ...
        mode_t mode = fRecordKext ? (S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH) : (S_IXUSR|S_IRUSR|S_IWUSR);
        int argsFile = open(path, O_WRONLY|O_CREAT|O_TRUNC | O_BINARY, mode);
...
void Snapshot::recordArch(const char *arch)
{
    ...
    if (!archInArgs) {
        if (fRootDir == NULL) {
            ...
            mode_t mode = fRecordKext ? (S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH) : (S_IRUSR|S_IWUSR);
            int fd=open(path_buf, O_WRONLY|O_CREAT|O_TRUNC | O_BINARY, mode);
    ...
    if (fRootDir == NULL) {
        ...
        if (fRecordObjects) {
            ...
            if (fFilelistFile == -1) {
                ...
                mode_t mode = fRecordKext ? (S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH) : (S_IRUSR|S_IWUSR);
                fFilelistFile = open(filelist_path, O_WRONLY|O_CREAT|O_TRUNC | O_BINARY, mode);
    ...
    if (fRootDir == NULL) {
        ...
        if (fRecordDylibSymbols) {
            ...
            if (it == fDylibSymbols.end()) {
                ...
                mode_t mode = fRecordKext ? (S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH) : (S_IRUSR|S_IWUSR);
                dylibFd = open(path_buf, O_WRONLY|O_APPEND|O_CREAT | O_BINARY, mode);
...
void Snapshot::recordAssertionMessage(const char *fmt, ...)
{
    ...
    if (msg != NULL) {
        if (fRootDir == NULL) {
            ...
            mode_t mode = fRecordKext ? (S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH) : (S_IRUSR|S_IWUSR);
            int log = open(path, O_WRONLY|O_APPEND|O_CREAT | O_BINARY, mode);
