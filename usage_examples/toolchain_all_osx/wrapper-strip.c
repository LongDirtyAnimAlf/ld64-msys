...
int main(int argc, char *argv[])
{
    ...
    target_info(argv, &target, &dummylinker);

#if defined(WINDOWS) || defined(_WIN32) || defined(__CYGWIN__)
    const char linker[PATH_MAX+1] = "llvm-objcopy.exe";
#else
    const char linker[PATH_MAX+1] = "llvm-objcopy";
#endif
    ...
    fprintf(stderr, "cannot invoke strip [llvm-objcopy]!\n");
